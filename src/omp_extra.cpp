...
template void reduce(uint64_t **arrs, uint64_t bins, uint64_t begin,
                     uint64_t end);
template void reduce(uint8_t **arrs, uint64_t bins, uint64_t begin,
                     uint64_t end);

int physical_n_threads() { return std::thread::hardware_concurrency(); }
